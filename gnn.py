...
#     return result
@app.get("/user_id")
async def gnn_predict(user_id: int):
    # 新しいHeteroDataインスタンスを作成
    new_data = HeteroData()
    hidden_sizes=64
    dropout_rate=0.0
    num_user_features=17
    ...
    try:
        ...
        # モデルの読み込み
        model = Model(hidden_sizes, dropout_rate, new_data, num_user_features, num_content_features, num_users, num_contents)
        model.load_state_dict(torch.load(model_path, map_location=torch.device('cpu')))
        model.eval()
        ...
        # ユーザーに対するレコメンドIDの取得
        result = recommend_user(user_id, prediction, top_k=3)
